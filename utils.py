...
def image_save(outputs_seg, fake_mask_path, ori_shape, color=False):
    outputs_seg_save = outputs_seg.clone().detach().cpu()
    if color:
        outputs_seg_save = F.interpolate(outputs_seg_save, (ori_shape[0], ori_shape[1]), mode="bilinear")
    else:
        outputs_seg_save = F.interpolate(outputs_seg_save, (ori_shape[0], ori_shape[1]), mode="nearest")
    outputs_seg_save = outputs_seg_save.numpy()
    outputs_seg_save = outputs_seg_save[0, 0, :, :]
    # outputs_seg_save = np.round(outputs_seg_save)
    outputs_seg_save = outputs_seg_save * 255
    outputs_seg_save = outputs_seg_save.astype("uint8")

    # fake_mask_path2 = fake_mask_path.replace('.','_')
    # fake_mask_list = list(fake_mask_path2)
    # fake_mask_list[-4] = '.'
    # fake_mask_path2 = "".join(fake_mask_list)

    if color:
        image = cv2.applyColorMap(outputs_seg_save, cv2.COLORMAP_JET)
        cv2.imwrite(fake_mask_path, image)
    else:
        outputs_seg_save = Image.fromarray(outputs_seg_save)
        imsave(fake_mask_path, outputs_seg_save)
